...
Risolve i bug di tokenizzazione e parsing identificati.
"""
...
class FixedLambdaParser:
    """Parser corretto per espressioni lambda calculus."""
    ...
    def _tokenize(self, expression: str) -> List[str]:
        ...
        # CORREZIONE BUG: Aggiungi spazi tra TUTTE le variabili adiacenti
        # Prima identifica le variabili e aggiungi spazi tra loro
        # Ma NON tra variabili che sono già separate da spazi o punti
        expr = re.sub(r'([a-zA-Z])([a-zA-Z])', r'\1 \2', expr)
        
        # Aggiungi spazi attorno a parentesi e lambda per separare i token
        expr = expr.replace('(', ' ( ')
        expr = expr.replace(')', ' ) ')
        expr = expr.replace('\\', ' \\ ')
        expr = expr.replace('.', ' . ')
        
        # Splitta su spazi e rimuovi token vuoti
    ...
    def _parse_application(self) -> Term:
        ...
        while (self.position < len(self.tokens) and 
               ...
               self.tokens[self.position] not in 'λ' and
...
def test_fixed_parser():
    """Testa il parser corretto."""
    parser = FixedLambdaParser()
    reducer = FixedBetaReducer()
